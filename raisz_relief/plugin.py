...
class RaiszReliefPlugin:
    ...
    def initProcessing(self):
        ...
        # Grouped dialogs ("Configure..."): register our parameter type and
        # widget factory. Wrapped -- on failure the plugin must keep working
        # exactly as before, simply without the grouping buttons.
        try:
            from .algorithms import ui_binding
            ui_binding.register()
        except Exception as e:
            QgsMessageLog.logMessage(
                "grouped dialogs not wired: %s" % e, "RaiszRelief")
    ...
    def open_dialog(self):
        try:
            ...
            # fallback: if the algorithm dialog is unavailable, just open
            # the Processing panel so the user can find the plugin
            try:
                ...
                QgsMessageLog.logMessage(
                    "Could not open the Processing panel: %s" % e,
                    "Raisz-style Relief", Qgis.MessageLevel.Warning)

    def unload(self):
        # removed symmetrically: otherwise a factory from the previous
        # session is left hanging in the registry after a reinstall
        try:
            from .algorithms import ui_binding
            ui_binding.unregister()
        except Exception as e:
            QgsMessageLog.logMessage(
                "removing grouped dialogs: %s" % e, "RaiszRelief")
