...
def build_base_fill(mode, z, disp, illum, grid, *, palette="patterson",
                    ...
                    watercolor=0.0, wc_masks=None, wc_log=None):
    ...
    if rgba is not None and watercolor > 0.0:
        # BEFORE draping: grain and the wet edge belong to the sheet, but
        # applied after the displacement they would land on top of the
        # hachuring and drift away from the fill they outline
        from .watercolor import watercolorize, WC_GLAZES
        rgba = watercolorize(rgba, strength=watercolor, masks=wc_masks,
                             glazes=WC_GLAZES, log=wc_log)
    if rgba is None:
        return None, None
    if valid is not None:
        rgba = rgba.copy()
...
def build_cover_masks(overlays, grid, affine=None, out_shape=None):
    """Rasterise land-cover and water polygons into 0..1 masks for the
    watercolour pass.

    Forest, sand, ice, scrub and grass are currently drawn as SYMBOLS over
    the hachuring (compose.draw_landcover), while lakes, seas and marshes
    are area fills with their own patterns. The symbols stay where they
    are: the watercolour does not replace them, it lays a colour wash
    UNDER the relief, the way a glaze sits under a pen drawing on paper.
    So the masks are built from the same source polygons but consumed by a
    ...
    aff = affine if affine is not None else grid.affine
    out = {}
    for key in ("forest", "marsh", "scrub", "grass", "sand", "ice",
                "lake", "sea"):
        items = list(overlays.get(key) or [])
        if key == "sea":
            # auto-sea (AUTO_SEA / zero level) is the same body of water
            items += list(overlays.get("sea_auto") or [])
        shapes = []
        for it in items:
            rings = as_rings(it)
            if not rings:
            ...
            try:
                shapes.append(({"type": "Polygon",
                                "coordinates": [np.asarray(r).tolist()
                                                for r in rings]}, 1))
            except Exception as e:
                QgsMessageLog.logMessage(
                    "watercolor: polygon skipped in %s: %s" % (key, e),
                    "RaiszRelief")
                continue
        if not shapes:
            continue
        m = rasterize(shapes, out_shape=(ny, nx), transform=aff,
                      fill=0, dtype="uint8", all_touched=True)
        if m.any():
            out[key] = m.astype("float64")
    return out


def build_water_fill(overlays, styles, grid, disp, *, affine=None,
                     out_shape=None, strength=1.0, log=None):
    """Watercolour layer for area waters -- seas, lakes, marshes.

    Why separate from the base fill. Waters are NOT drawn inside the fill
    pipeline; they are polygons laid over it (compose.draw_area_waters,
    alpha ~0.92). A water glaze applied inside build_base_fill therefore
    ended up under a near-opaque patch and was invisible: the log said
    "glaze sea over 78% of the area" while the sheet showed flat blue.

    Here the water is assembled into its own RGBA, watercolourised and
    draped by the same disp. The flat patches in compose are switched off
    (faces=False), while the shoreline stroke and the patterns stay.

    The tone is uneven: denser at the shore, lighter towards the middle of
    large bodies. Computed from a distance transform of the water mask,
    normalised by ITS OWN maximum, so a small pond does not wash out in the
    middle while a sea does.
    """
    from rasterio.features import rasterize
    from scipy import ndimage as _ndi
    from .watercolor import (WC_GLAZES, watercolorize, srgb_to_linear,
                             linear_to_srgb, apply_density)
    ...
    for key in ("sea", "lake", "marsh"):
        ...
        if not shapes:
            continue
        m = rasterize(shapes, out_shape=(ny, nx), transform=aff, fill=0,
                      dtype="uint8", all_touched=True).astype("float64")
        if not m.any():
            continue
        any_water = True
        masks[key] = m
        sty = styles.get(key, {})
        face = np.asarray(_hex2rgb(sty.get("face", "#a4cbe6")), "float64")
        rgba[..., :3] = np.where(m[..., None] > 0, face, rgba[..., :3])
        rgba[..., 3] = np.maximum(rgba[..., 3], m * float(sty.get("alpha", 0.9)))
    if not any_water:
        return None, None

    lin = srgb_to_linear(rgba[..., :3])
    for key, m in masks.items():
        d = _ndi.distance_transform_edt(m > 0)
        dmax = float(d.max())
        if dmax > 3.0:
            # denser at the shore, lighter towards the middle; normalising
            # by its OWN maximum keeps a pond beside a sea from washing out
            t = np.clip(d / dmax, 0.0, 1.0)
            dens = 1.0 + 0.45 * strength * (1.0 - 2.0 * t)
            lin = np.where(m[..., None] > 0, apply_density(lin, dens), lin)
    rgba[..., :3] = linear_to_srgb(lin)

    rgba = watercolorize(rgba, strength=strength, masks=masks,
                         glazes={k: WC_GLAZES[k] for k in masks
                                 if k in WC_GLAZES}, log=log)
    return drape_image(rgba, disp)
