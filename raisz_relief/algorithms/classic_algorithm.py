...
class ClassicAlgorithm(ReliefAlgorithmBase):
    ...
    VALLEY_BREAK = "VALLEY_BREAK"; FLAT_SHORT = "FLAT_SHORT"
    LONELY = "LONELY"; CAST_SHADOW = "CAST_SHADOW"
    ...
    def initAlgorithm(self, config=None):
        ...
        adv = [
            ...
            (self.VALLEY_BREAK,
             "Break strokes in thalwegs: strength 0-1 (0 = off)", D, 0.7,
             "Stops a stroke where flow lines converge closer than the "
             "stroke spacing, so strokes from opposite slopes no longer "
             "fuse into dark blots along the valley floor.",
             0.0, 1.0),
            (self.FLAT_SHORT,
             "Shorten strokes on gentle ground: strength 0-1 (0 = off)",
             D, 0.6,
             "Scales the descent at which a stroke breaks. On gentle "
             "ground the stroke is otherwise as long as the bench spacing "
             "and reads as a scratch across empty paper.",
             0.0, 1.0),
            (self.LONELY,
             "Thin out stray strokes: strength 0-1 (0 = off)", D, 0.4,
             "Removes strokes whose surroundings describe too few contour "
             "levels. A stroke on a single contour depicts no form. "
             "0.4 needs 2 levels, 1.0 needs 4.",
             0.0, 1.0),
            (self.CAST_SHADOW,
             "Cast shadow from the terrain: strength 0-1 (0 = off)",
             D, 0.5,
             "Darkens ground that a neighbouring ridge hides from the sun. "
             "Azimuth and altitude come from the light parameters above.",
             0.0, 1.0),
            (self.WATERCOLOR,
             "Watercolour the fill: strength 0-1 (0 = off)", D, 0.0,
             "Adds paper grain, a wet edge and glazes for land cover and "
             "waters. Requires a fill to be enabled.",
             0.0, 1.0),
        ...
        for t in adv:
            nm, lbl, typ, dv, hlp = t[0], t[1], t[2], t[3], t[4]
            if len(t) == 7:      # with bounds: the 0-1 strengths (see 7.4.1)
                self.addParameter(self._h(self._adv(
                    QgsProcessingParameterNumber(
                        nm, self.tr(lbl), typ, dv,
                        minValue=t[5], maxValue=t[6])), hlp))
                continue
        ...
        # Grouped dialogs: five service button parameters. Added AFTER all
        # the ordinary ones -- the wrapper looks its neighbours up by name,
        # so they must exist by the time postInitialize runs.
        try:
            from . import ui_binding
            ui_binding.add_group_params(self)
        except Exception as e:
            QgsMessageLog.logMessage(
                "parameter groups not added: %s" % e, "RaiszRelief")
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if strip_rows > 0:
            ...
            stats = cs.render_classic_striped(
                ...
                valley_break=gd(self.VALLEY_BREAK),
                flat_short=gd(self.FLAT_SHORT), lonely=gd(self.LONELY),
                cast_shadow=gd(self.CAST_SHADOW),
                watercolor=gd(self.WATERCOLOR),
