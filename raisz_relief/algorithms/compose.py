...
def blit_fill(ax, img, extent, z=1, smooth=True):
    """smooth=False -- output without interpolation: paper grain is finer
    than three pixels, and bilinear filtering would eat it with the noise."""
    if img is not None:
...
def draw_area_waters(ax, overlays, geff, disp, styles, z0=2, pat=None,
                     vis=None, hidden=None, faces=True):
    ...
    # faces=False: the water fill arrives as a separate watercolour raster
    # (fills.build_water_fill); only the shoreline stroke and the patterns
    # remain here. Otherwise a patch at alpha ~0.92 would cover the texture.
    n = {}
    for key, sty in (("sea", styles["sea"]), ("marsh", styles["marsh"]),
                     ("lake", styles["lake"])):
        n[key] = draw_polys(ax, _vp(overlays.get(key, [])), geff, disp, z=z0,
                            face=(sty["face"] if faces else "none"),
                            edge=sty["edge"],
    ...
    if sea_auto:
        ...
        n["sea_auto"] = draw_poly_holes(
            ax, _vp(sea_auto), geff, disp, z=z0,
            face=(s_sty["face"] if faces else "none"),
...
def draw_thematic_lines(ax, overlays, geff, disp, z=7.5, vis=None):
    """Thematic line layer: colour, width and dashing come from the layer
    style, clipped where the terrain hides it (as roads are). Drawn above
    the hachuring."""
    n = 0
    for coords, color, width_mm, dash in overlays.get("thematic_line", []):
        lw = max(0.3, float(width_mm) / 0.35)   # mm -> approx. points
        n += draw_lines(ax, [coords], geff, disp, color=color, lw=lw, z=z,
                        linestyle=dash, vis=vis)
    return n
