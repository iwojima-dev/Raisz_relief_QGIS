...
def _line_dash(sym):
    """Symbol line style -> matplotlib linestyle. Best-effort: anything
    unexpected falls back to a solid line."""
    try:
        sl = sym.symbolLayer(0)
        ps = int(sl.penStyle()) if hasattr(sl, "penStyle") else 1
        return {1: "-", 2: "--", 3: ":", 4: "-.", 5: "-."}.get(ps, "-")
    except Exception:  # nosec B110 -- dashing is optional
        return "-"


def extract_thematic_lines(layer, to_crs, extent=None,
                           default_color="#333333"):
    """Thematic lines -> list of (coords(N,2), '#rrggbb', width_mm, dash).

    Colour, width and dashing come from the QGIS layer style under any
    renderer (symbolsForFeature, per feature). Clipping behind mountains
    happens later in the core through vis, as it does for roads. dash is a
    matplotlib style ('-', '--', ':', '-.')."""
    if layer is None:
        return []
    tr = _xform(layer, to_crs); rect = _rect_geom(extent)
    ...
    try:
        for f in layer.getFeatures():
            color, width, dash = default_color, 0.6, "-"
            if renderer is not None:
                try:
                    syms = renderer.symbolsForFeature(f, ctx)
                    if syms:
                        s = syms[0]
                        color = s.color().name()
                        width = float(s.width()) or 0.6
                        dash = _line_dash(s)
                except Exception:  # nosec B110 -- the style is optional,
                    pass           # the defaults above are kept
            g = _prep(f.geometry(), tr, rect)
            if g is None:
                continue
            parts = g.asMultiPolyline() if g.isMultipart() else [g.asPolyline()]
            for part in parts:
                if part and len(part) >= 2:
                    res.append((_arr(part), color, width, dash))
    finally:
        if renderer is not None:
            try:
                renderer.stopRender(ctx)
            except Exception:  # nosec B110 -- renderer teardown is best-effort
                pass
    return res


def layer_fill_color(layer):
    """A representative colour of a layer taken from its style (the first
    symbol of the renderer), or None. Used by the "style from layer"
    checkbox on area layers: complex renderers (categorized, graduated) are
    reduced to their first symbol."""
    if layer is None:
        return None
    try:
        r = layer.renderer()
        syms = r.symbols(QgsRenderContext()) if r is not None else None
        if syms:
            return syms[0].color().name()
    except Exception:  # nosec B110 -- a colour from the style is optional
        pass
    return None
