...
# Qt6/PyQt6 requires fully qualified (scoped) enums. The minimum supported
# QGIS is 3.40, where these forms exist natively, so no Qt5 try/except
# fallback is needed. Define short aliases once and use them in all three
# algorithm files.
NUM_DOUBLE = QgsProcessingParameterNumber.Type.Double
NUM_INT = QgsProcessingParameterNumber.Type.Integer
FLAG_ADVANCED = QgsProcessingParameterDefinition.Flag.Advanced
...
class ReliefAlgorithmBase(QgsProcessingAlgorithm):
    ...
    STYLE_FROM_LAYER = "STYLE_FROM_LAYER"
    SETTLE_PT = "SETTLE_PT"
    SETTLE_POLY = "SETTLE_POLY"
    SETTLE_LABEL = "SETTLE_LABEL"
    SETTLE_FONT = "SETTLE_FONT"; SETTLE_FONT_SCALE = "SETTLE_FONT_SCALE"
    # generic matplotlib families: available on any system, unlike named
    # typefaces that may silently fall back on another machine
    SETTLE_FONTS = ["Default", "Serif", "Sans-serif", "Monospace",
                    "Cursive", "Fantasy"]
    SETTLE_FONT_KEYS = [None, "serif", "sans-serif", "monospace",
                        "cursive", "fantasy"]
    ...
    PAPER_INK = [
        ...
        ("#e9bc8e", "#7a4859"),   # diazotype: warm ochre + plum ink (from scan)
    ]
    ...
    def add_overlay_params(self):
        ...
        self.addParameter(self._vec(
            self.THEMATIC_LINE,
            "Thematic line layer (lines; colour/width/dash from the style)",
            [SRC_LINE],
            "Drawn above the hachuring in the colour, width and dash "
            "pattern of the layer style, clipped where the terrain hides "
            "it. The style is always used here, regardless of the "
            "\"styling from layer\" checkbox below."))
        ...
        self.addParameter(self._h(QgsProcessingParameterEnum(
            self.SETTLE_FONT, self.tr("Settlement label font"),
            options=self.SETTLE_FONTS, defaultValue=0),
            "Font family for settlement labels. Generic matplotlib families "
            "(serif, sans-serif, monospace, cursive, fantasy) are used rather "
            "than named typefaces, so they resolve on any system."))
        self.addParameter(self._h(QgsProcessingParameterNumber(
            self.SETTLE_FONT_SCALE,
            self.tr("Settlement label and marker size (1.0 = auto by sheet)"),
            NUM_DOUBLE, 1.0, minValue=0.2, maxValue=5.0),
            "Multiplier over the automatic, sheet-relative size. The label "
            "size scales with the sheet instead of being a fixed point value, "
            "so place names no longer shrink on large DEMs; the marker, its "
            "offset and halo scale with it."))
        ...
        self.addParameter(self._h(QgsProcessingParameterBoolean(
            ...
            self.tr("Styling from layer styles (colour of waters and land "
                    "cover from the QGIS layer style, not from the theme)"),
            False),
            "Off by default: colours come from the paper/fill theme. When "
            "on, area waters (seas, lakes, marshes) and land cover "
            "(forest, sand, ice, scrub, grassland) take the colour of "
            "their own layer style -- the first symbol of the renderer. "
            "Our patterns and textures are kept; only the colour changes."))
    ...
    def fill_kwargs(self, parameters, context):
        ...
        return dict(
            ...
            settle_font=self.SETTLE_FONT_KEYS[
                self.parameterAsEnum(parameters, self.SETTLE_FONT, context)],
            settle_font_scale=self.parameterAsDouble(
                parameters, self.SETTLE_FONT_SCALE, context),
    ...
    def build_overlays(self, parameters, context, dem_crs, grid, fill_mode,
        ...
        o["thematic_line"] = ov.extract_thematic_lines(
            gv(self.THEMATIC_LINE), dem_crs, ext)
        if self.parameterAsBool(parameters, self.STYLE_FROM_LAYER, context):
            # colour from the layer styles (first symbol of the renderer);
            # None where the layer is absent or has no colour -- in
            # apply_style_overrides the theme colour stays there
            o["style_colors"] = {
                "lake": ov.layer_fill_color(gv(self.LAKES)),
                "sea": ov.layer_fill_color(gv(self.SEAS)),
                "marsh": ov.layer_fill_color(gv(self.MARSHES)),
                "forest": ov.layer_fill_color(gv(self.LC_FOREST)),
                "sand": ov.layer_fill_color(gv(self.LC_SAND)),
                "ice": ov.layer_fill_color(gv(self.LC_ICE)),
                "scrub": ov.layer_fill_color(gv(self.LC_SCRUB)),
                "grass": ov.layer_fill_color(gv(self.LC_GRASS)),
            }
        return o
