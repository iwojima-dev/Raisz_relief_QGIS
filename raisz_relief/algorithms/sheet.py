...
def _crs_transformers(proj_wkt):
    """(to WGS84, from WGS84, is-geographic flag) for the raster CRS.

    Inside QGIS we use its own transformer. pyproj, called from a
    Processing worker thread, touches the PROJ database in a foreign
    context and takes down the WHOLE process (access violation in
    DatabaseContext) -- a native crash that try/except cannot catch.
    Verified: QGIS matches pyproj to 9 decimal places, 4000 points in
    9 ms. pyproj stays as the fallback for use outside QGIS.
    """
    try:
        from qgis.core import (QgsCoordinateReferenceSystem,
                               QgsCoordinateTransform,
                               QgsCoordinateTransformContext, QgsPointXY)
        src = QgsCoordinateReferenceSystem.fromWkt(proj_wkt)
        if not src.isValid():
            raise ValueError("CRS not recognised by QGIS")
        dst = QgsCoordinateReferenceSystem.fromEpsgId(4326)
        ctx = QgsCoordinateTransformContext()

        def _mk(a, b):
            t = QgsCoordinateTransform(a, b, ctx)

            def _f(x, y):
                xs = np.atleast_1d(np.asarray(x, dtype="float64")).ravel()
                ys = np.atleast_1d(np.asarray(y, dtype="float64")).ravel()
                ox = np.empty(xs.size); oy = np.empty(xs.size)
                for i in range(xs.size):
                    p = t.transform(QgsPointXY(float(xs[i]), float(ys[i])))
                    ox[i] = p.x(); oy[i] = p.y()
                return ox, oy
            return _f
        return _mk(src, dst), _mk(dst, src), bool(src.isGeographic())
    except ImportError:
        from pyproj import CRS, Transformer
        crs = CRS.from_user_input(proj_wkt)
        wgs = CRS.from_epsg(4326)
        fwd = Transformer.from_crs(crs, wgs, always_xy=True)
        inv = Transformer.from_crs(wgs, crs, always_xy=True)
        return (lambda x, y: fwd.transform(x, y),
                lambda x, y: inv.transform(x, y), bool(crs.is_geographic))


def _lonlat_fn(proj_wkt):
    """Raster CRS -> function (x, y) -> (lon, lat) WGS84."""
    fwd, _inv, geographic = _crs_transformers(proj_wkt)
    if geographic:
...
def _north_vec(geff, proj_wkt, rows, cols):
    ...
    fwd, inv, geographic = _crs_transformers(proj_wkt)
    c, r = cols / 2.0, rows / 2.0
    x0, y0 = _pix2world(c, r, geff)
    if geographic:
        x1, y1 = float(x0), float(y0) + 0.01
    else:
        lon, lat = fwd(x0, y0)
        x1, y1 = inv(lon, np.asarray(lat, float) + 0.01)
        x1 = float(np.atleast_1d(x1)[0]); y1 = float(np.atleast_1d(y1)[0])
    g = geff
    det = g[1] * g[5] - g[2] * g[4]
    dx, dy = x1 - x0, y1 - y0
