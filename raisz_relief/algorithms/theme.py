...
def apply_style_overrides(styles, style_colors):
    """The "style from layer" checkbox: replace the colours of area waters
    and land cover with the colours of their own layer styles.
    style_colors: {key: '#rrggbb'|None}. None and missing keys are left
    alone -- the theme colour (the default) stays there. Patterns and
    textures are preserved; only the colour changes. The dictionaries are
    copied so the shared theme template is not damaged."""
    if not style_colors:
        return styles
    for k in ("sea", "lake", "marsh"):
        c = style_colors.get(k)
        if c and k in styles:
            styles[k] = dict(styles[k]); styles[k]["face"] = c
    lc = styles.get("landcover")
    if lc:
        lc = dict(lc)
        for k in ("forest", "sand", "ice", "scrub", "grass"):
            c = style_colors.get(k)
            if c and k in lc:
                lc[k] = dict(lc[k]); lc[k]["color"] = c
        styles["landcover"] = lc
    return styles
