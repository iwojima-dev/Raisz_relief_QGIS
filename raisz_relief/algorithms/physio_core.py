...
def _wc_aff(geff):
    """Rotated georeference -> Affine for rasterising the cover masks."""
    from affine import Affine
    return Affine.from_gdal(*geff)
...
def render_landform(
    ...
    # selector (zg = base + relief*lift), then compute slopes/light/
    # contours/stroke from zg. disp keeps the *w_relief (Hammond) factor,
    # the fill is coloured by the REAL z0. The old disp*=k stretched a
    # finished gentle drawing instead of drawing the form.
    if rel_scale:
        disp0 = (relief * shear / py) * w_relief
        # selector: lift sharp small forms (scarps, cliffs), not plains,
        # broad gentle hills or pixel DEM noise
        w = gridmod.steep_weight(z, px, py, base_scale_px=base_scale_px,
                                 log=lambda m: tick(31, m))
        ...
        zg = z
        relief_g = relief
    disp = (relief_g * shear / py) * w_relief
    ...
    if rel_scale:
        # slope normalization folded into rel_scale: p95 of tan in the
        # relief zone -> 1.0 (Hammond/LR untouched)
        ...
        if _sr.size:
            ...
            tick(33, "Relative slopes: p95(tan)=%.3f -> 1.0" % _top)
    ...
    if conv is not None:
        tick(20, "Flow convergence: threshold %.4g 1/m (radius %.0f m)"
             % (conv_thr, 1.0 / conv_thr))
    gridmod.scene_report(zg, slope, px, py, interval, 0.0,
                         lambda m: tick(21, m))
    v = (illum + 1) / 2.0
    if cast_shadow > 0.0:
        # diffuse illumination and terrain occlusion are different
        # quantities: the second falls on lit slopes as well
        _cast = gridmod.cast_shadow(zg, px, py, light_az, light_alt)
        v = np.clip(v - cast_shadow * 0.6 * _cast, 0.0, 1.0)
        tick(22, "Cast shadow: %.0f%% of the area (azimuth %.0f deg, "
    ...
    img, extent = fills.build_base_fill(
        ...
        wc_masks=(fills.build_cover_masks(
            ...
            out_shape=(z0.shape if view_rot else None))
    ...
    hidden = compose.hidden_geometry(vis, geff, log=lambda m: tick(41, m))
    if watercolor > 0.0:
    ...
    if draw_framework:
        ...
        for seg in contour_paths:
            ...
            # contiguous runs as one polyline (see classic_core)
            d = np.diff(np.concatenate(([0], keep.astype(np.int8), [0])))
            for a_, b_ in zip(np.flatnonzero(d == 1), np.flatnonzero(d == -1)):
                if b_ - a_ >= 2:
                    frame_list.append(
                        np.column_stack((c_idx[a_:b_], r_new[a_:b_])))
        if frame_list:
            n_frame = len(frame_list)
            ax.add_collection(LineCollection(
                frame_list, linewidths=0.3, colors=[(*ink_rgb, 0.4)],
                zorder=4))
    ...
    if draw_fall:
        ...
        def trace_fall(r0, c0, drop_limit):
            ...
            for _ in range(max_steps):
                ...
                # -- stop at the foot, as a stroke depicting a SLOPE should
                if conv is not None and conv[int(r), int(c)] > conv_thr:
                    break
                if z0v - float(smp(zg, r, c)[0]) >= drop_limit:
                    break
            return np.array(pts)
        ...
        for k, seg in enumerate(contour_paths):
            ...
            while s < dlen[-1]:
                ...
                # in a convergence zone damp the "darkness" term: a thalweg
                # is shaded and concave at once, so denser seeding, extra
                # width and higher opacity all piled up into one blot there
                cv = 0.0
                if conv is not None:
                    cv = float(np.clip(
                        conv[int(ri), int(ci)] / conv_thr, 0.0, 1.0))
                dark = (1.0 - vv) * (1.0 - 0.7 * cv)
                s += max(fall_spacing * (1.0 - 0.5 * dark) * (1.0 - 0.3 * sr),
                         fall_spacing * 0.3)
                if wr < 0.5:        # strokes begin where the stipple ends
                    continue
                W = (sr * max_width * slope_weight
                     + dark * max_width * (1.0 - slope_weight)) * wr
                if W < min_draw_width:
                    continue
                if cv >= 1.0:       # seed inside the convergence funnel
                    continue
                drop_limit = (interval
                              * (1.0 + 1.4 * max(0.0, (0.66 - wr) / 0.16))
                              * gridmod.drop_fraction(sr, flat_short))
                fall = trace_fall(ri, ci, drop_limit)
                if len(fall) < 2:
                    continue
                r_new = fall[:, 0] - smp(disp, fall[:, 0], fall[:, 1])
                vmask = vis_at(fall[:, 0], fall[:, 1])
                # contiguous visible runs of the stroke (see classic_core)
                _d = np.diff(np.concatenate(([0], vmask.astype(np.int8), [0])))
                runs = [(x, y) for x, y in zip(np.flatnonzero(_d == 1),
                                               np.flatnonzero(_d == -1))
                        if y - x >= 2]
                if not runs:
                    continue
                ...
                bench_list.append(gridmod.bench_spacing_px(
                    float(slope[int(ri), int(ci)]), interval, (px + py) / 2.0))
                a = min(0.45 + 0.5 * dark, 0.95)
                # a list item = ALL paths of one stroke: the single-stroke
                # thinning below works by stroke index
                if hand_jitter > 0:
                    ...
                    wall = np.clip(W * 0.5 * (prof[:-1] + prof[1:]),
                                   min_draw_width * 0.4, None)
                    g_seg, g_lw = [], []
                    for x, y in runs:      # pen pressure -- per edge
                        pts = np.column_stack((fall[x:y, 1], r_new[x:y]))
                        for i in range(len(pts) - 1):
                            g_seg.append(pts[i:i + 2])
                            g_lw.append(float(wall[x + i]))
                else:
                    g_seg = [np.column_stack((fall[x:y, 1], r_new[x:y]))
                             for x, y in runs]
                    g_lw = [W] * len(g_seg)
                seg_list.append(g_seg)
                lw_list.append(g_lw)
                col_list.append([(*ink_rgb, a)] * len(g_seg))
        if seg_list and lonely > 0.0:
            # stray-stroke thinning (since 7.4.2 shared by all three cores,
            # measured by the number of contour levels around a stroke)
            _keep = gridmod.sparse_levels_mask(
                seed_list, lev_list, bench_list, lonely,
                log=lambda m: tick(88, m))
            if not _keep.all():
        ...
        if seg_list:
            flat_seg = [p for g in seg_list for p in g]
            flat_lw = [w for g in lw_list for w in g]
            flat_col = [c for g in col_list for c in g]
            n_fall = len(flat_seg)
            ax.add_collection(LineCollection(
                flat_seg, linewidths=flat_lw, colors=flat_col,
    ...
    if _ext in (".pdf", ".svg", ".eps"):
        tick(97, "Writing %s: %d stroke paths, %d framework -- this may take "
